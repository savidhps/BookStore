...
import { deleteAUserBookApi, getAllUsersBookApi, getAllUsersBroughtBookApi, uploadBookApi } from '../../services/allApi';
...
function Profile() {
  ...
  const getallUserBook = async () => {
    ...
    const result = await getAllUsersBookApi(reqHeader)
    console.log(result);
  ...
  const getallUserBroughtBook = async () => {
    ...
    const result = await getAllUsersBroughtBookApi(reqHeader)
    console.log(result);
    if (result.status == 200) {
  ...
  return (
    <>
      ...
      {/* Contents  */}
      {sellStatus &&
        <div className='bg-gray-200 rounded-lg mx-10 md:p-10 mb-5 '>
      ...
      {bookStatus && <div className='p-10 my-20 shadow rounded-lg '>
        ...
        <div className='bg-gray-200 p-4 rounded mt-5' key={index}>
          <div className='md:grid grid-cols-[3fr_1fr]'>
            ...
            <div>
            {/* add image after class*/}
              <img src={item?.imageurl} alt="no img" className='w-full' />
              <div className='flex justify-end mt-4'>
                <button onClick={()=>deleteBook(item?._id)} className='px-3 py-2 bg-red-600 text-white hover:bg-white hover:text-red-600  
                hover:border hover:border-red-600 rounded-md'>Delete</button>
              </div>
            </div>
          </div>
        </div>
    ...
    :
      ...
      {purchaseStatus &&
        ...
        <div className='bg-gray-200 p-4 rounded ' key={index}>
          <div className='md:grid grid-cols-[3fr_1fr]'>
            <div className='px-4'>
              <h1 className='text-2xl font-bold'>{item?.title}</h1>
              <h2>Bram Stoker</h2>
